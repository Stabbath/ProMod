...
//use struct with model names and damage values. hook con var changes to update it. convert modelname into index for struct and get damage. <- yes?
...
public Action:OnTakeDamage( victim, &attacker, &inflictor, &Float:damage, &damageType, &weapon, Float:damageForce[3], Float:damagePosition[3] )
{
	...
	if (StrEqual(sClass,"prop_physics"))
	{
		...
		if (bIsBridge)	//too many ifs and elses, and else ifs are evil
		{
		...
		else
		{
			...
			if (GetEntProp(victim, Prop_Send, "m_isIncapacitated") && val >= 0.0)	//make it so that negative values just make damage default to the hittable's standing damage
			{
			...
			{	
				...
				else
				{
					...
					else
					{
						...
						{
							if (StrEqual(sModelName, "models/props_fairgrounds/bumpercar.mdl"))
							{
